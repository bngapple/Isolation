...
class SignalEngine:
    ...
    def _eval_ib(self, state: MarketState) -> Optional[Signal]:
        ...
        # Original Hybrid v2 logic only trades post-IB through 15:30 ET.
        bar = state.last_bar
        if bar.timestamp.time() >= dt_time(15, 30):
            return None

        # Original backtest skipped very narrow opening ranges.
        if ib.range < 2.0:
            return None
        ...
        if bar.high > ib.high:
            side = Side.BUY
            reason = f"IB Breakout UP — high {bar.high:.2f} > IB high {ib.high:.2f}"
        elif bar.low < ib.low:
            side = Side.SELL
            reason = f"IB Breakout DOWN — low {bar.low:.2f} < IB low {ib.low:.2f}"
