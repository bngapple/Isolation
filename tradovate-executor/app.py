...
class TradovateExecutor:
    ...
    async def _on_bar_complete(self, state: MarketState):
        ...
        flatten_signals = [sig for sig in new_signals if sig.contracts == 0]
        entry_signals = [sig for sig in new_signals if sig.contracts > 0]

        for sig in flatten_signals:
            if sig.contracts == 0:
                # Flatten signal (max hold)
                await self._handle_flatten_signal(sig)

        selected_entry = self._select_account_entry(entry_signals)
        if selected_entry:
            self._pending_signals = [selected_entry]
            logger.info(
                f"QUEUED: {selected_entry.strategy} {selected_entry.side.value} {selected_entry.contracts} "
    ...
    def _has_account_level_position(self) -> bool:
        return any(not pos.is_flat for pos in self.signal_engine.positions.values())

    def _select_account_entry(self, entry_signals: list[Signal]) -> Optional[Signal]:
        selected, reason = select_account_entry(
            entry_signals,
            execution_cfg=self.config.execution,
            has_pending_entry=bool(self._pending_signals),
            has_open_position=self._has_account_level_position(),
        )
        if reason:
            prefix = "CONFLICT" if selected else "SKIPPED"
